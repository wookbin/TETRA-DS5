...
int power_read_data_all(int fd)
{
	...
	int m_ibattery = (packet_buf[4] & 0xff) | ((packet_buf[3] << 8) & 0xff00);
	m_data._dBattery_voltage = m_ibattery / 10.0;

	int m_system_cur = (packet_buf[6] & 0xff) | ((packet_buf[5] << 8) & 0xff00);
	m_data._dSystem_current = m_system_cur / 10.0;
	
	int m_Charge_cur = (packet_buf[8] & 0xff) | ((packet_buf[7] << 8) & 0xff00);
	m_data._dCharge_current = m_Charge_cur / 10.0;

	int _m_Charge_signal = (packet_buf[10] & 0xff) | ((packet_buf[9] << 8) & 0xff00);
	m_data._dCharge_signal = _m_Charge_signal / 10.0;
	
	int _m_Charge_voltage = (packet_buf[12] & 0xff) | ((packet_buf[11] << 8) & 0xff00);
	m_data._dCharge_voltage = _m_Charge_voltage / 10.0;

	int _m_Power1_voltage = (packet_buf[14] & 0xff) | ((packet_buf[13] << 8) & 0xff00);
	m_data._dPower1_voltage = _m_Power1_voltage / 10.0;
	
	int _m_Power2_voltage = (packet_buf[16] & 0xff) | ((packet_buf[15] << 8) & 0xff00);
	m_data._dPower2_voltage = _m_Power2_voltage / 10.0;

	int _m_Power3_voltage = (packet_buf[18] & 0xff) | ((packet_buf[17] << 8) & 0xff00);
	m_data._dPower3_voltage = _m_Power3_voltage / 10.0;

	/*
	Inport_status: packet_buf[19], packet_buf[20]
	Outport_status: packet_buf[21], packet_buf[22]
	Power_status: packet_buf[23], packet_buf[24]
	*/

	int _m_SBumper = (packet_buf[26] & 0xff) | ((packet_buf[25] << 8) & 0xff00);
	//m_data._iSBumper_0 = _m_SBumper;
	
	int _m_Sonar_0 = (packet_buf[28] & 0xff) | ((packet_buf[27] << 8) & 0xff00);
	m_data._dUltrasonic_0_distance = _m_Sonar_0 / 10.0;

	int _m_Sonar_1 = (packet_buf[30] & 0xff) | ((packet_buf[29] << 8) & 0xff00);
	m_data._dUltrasonic_1_distance = _m_Sonar_1 / 10.0;
	
	int _m_Sonar_2 = (packet_buf[32] & 0xff) | ((packet_buf[31] << 8) & 0xff00);
	m_data._dUltrasonic_2_distance = _m_Sonar_2 / 10.0;
	
	int _m_Sonar_3 = (packet_buf[34] & 0xff) | ((packet_buf[33] << 8) & 0xff00);
	m_data._dUltrasonic_3_distance = _m_Sonar_3 / 10.0;

	int _m_Sonar_4 = (packet_buf[36] & 0xff) | ((packet_buf[35] << 8) & 0xff00);
	m_data._dUltrasonic_4_distance = _m_Sonar_4 / 10.0;

	int _m_Sonar_5 = (packet_buf[38] & 0xff) | ((packet_buf[37] << 8) & 0xff00);
	m_data._dUltrasonic_5_distance = _m_Sonar_5 / 10.0;

	int _m_Sonar_6 = (packet_buf[40] & 0xff) | ((packet_buf[39] << 8) & 0xff00);
	m_data._dUltrasonic_6_distance = _m_Sonar_6 / 10.0;

	int _m_Sonar_7 = (packet_buf[42] & 0xff) | ((packet_buf[41] << 8) & 0xff00);
	m_data._dUltrasonic_7_distance = _m_Sonar_7 / 10.0;

	//*battery_Volt_H = packet_buf[3];
	//*battery_Volt_L = packet_buf[4];

	return ret;
}

int  power_read_Analog_data(int fd, int *iADB_0, int *iADB_1, int *iADB_2, int *iADB_3,
...
int  power_parameter_read(int fd, int *idata_0, int *idata_1, int *idata_2, int *idata_3,
                          int *idata_4, int *idata_5, int *idata_6, int *idata_7,
                          int *idata_8, int *idata_9, int *idata_10, int *idata_11, int *idata_12)
{
...
int power_set_OuputPort(int fd, int port0, int port1, int port2, int port3, int port4, int port5, int port6, int port7)
{
	int ret;
