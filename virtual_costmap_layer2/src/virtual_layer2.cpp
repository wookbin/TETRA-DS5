#include "virtual_costmap_layer2/virtual_layer2.hpp"
...
namespace virtual_costmap_layer2 {

// ---------------------------------------------------------------------

VirtualLayer2::VirtualLayer2() :
    _dsrv(nullptr)
{}
...
void VirtualLayer2::onInitialize()
{
    ...
    _dsrv = std::make_shared<dynamic_reconfigure::Server<VirtualLayer2Config>>(nh);

    dynamic_reconfigure::Server<VirtualLayer2Config>::CallbackType cb = boost::bind(&VirtualLayer2::reconfigureCb, this, _1, _2);
...
void VirtualLayer2::parseTopicsFromYaml(ros::NodeHandle &nh, const std::string &param)
{
        ...
        for (std::size_t i = 0; i < param_yaml.size(); ++i) {
            if (param_yaml[i].getType() != XmlRpc::XmlRpcValue::TypeString) {
                ...
                if (param.compare("zone_topics") == 0) {
                    _subs.push_back(nh.subscribe(topic_name, 100, &VirtualLayer2::zoneCallback, this));
                } else if (param.compare("obstacle_topics") == 0) {
                    _subs.push_back(nh.subscribe(topic_name, 100, &VirtualLayer2::obstaclesCallback, this));
                }
...
void VirtualLayer2::parseFormListFromYaml(const ros::NodeHandle &nh, const std::string &param)
{
    XmlRpc::XmlRpcValue param_yaml;
    if (nh.getParam(param, param_yaml)) {
...
void VirtualLayer2::convert(const XmlRpc::XmlRpcValue &val, geometry_msgs::Point &point)
{
    try {
...
bool VirtualLayer2::robotInZone(const Polygon &zone)
{
    if (!_one_zone_mode) {
...
void VirtualLayer2::reconfigureCb(VirtualLayer2Config &config, uint32_t level)
{
...
void VirtualLayer2::updateBounds(double robot_x, double robot_y, double robot_yaw,
...
void VirtualLayer2::updateCosts(costmap_2d::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
{
    if (!enabled_) {
        return;
    }
...
void VirtualLayer2::computeMapBounds()
{
    std::lock_guard<std::mutex> l(_data_mutex);
...
void VirtualLayer2::setPolygonCost(costmap_2d::Costmap2D &master_grid, const Polygon &polygon, unsigned char cost,
...
void VirtualLayer2::raytrace(int x0, int y0, int x1, int y1, std::vector<PointInt> &cells)
{
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    PointInt pt;
...
void VirtualLayer2::rasterizePolygon(const std::vector<PointInt> &polygon, std::vector<PointInt> &polygon_cells, bool fill)
{
...
void VirtualLayer2::zoneCallback(const virtual_costmap_layer2::Zone2ConstPtr &zone_msg)
{
...
void VirtualLayer2::obstaclesCallback(const virtual_costmap_layer2::Obstacles2ConstPtr &obstacles_msg)
{
    if (_clear_obstacles) {
...
geometry_msgs::Point VirtualLayer2::getRobotPoint()
{
