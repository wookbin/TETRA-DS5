...
#include "virtual_costmap_layer2/VirtualLayer2Config.h"
...
class VirtualLayer2 : public costmap_2d::Layer {
public:
    using Polygon = std::vector<geometry_msgs::Point>;
    VirtualLayer2();
    virtual ~VirtualLayer2();
...
private:
    ...
    void reconfigureCb(VirtualLayer2Config &config, uint32_t level);
    ...
    void zoneCallback(const virtual_costmap_layer2::Zone2ConstPtr &msg);

    /// \brief obstacle callback function
    void obstaclesCallback(const virtual_costmap_layer2::Obstacles2ConstPtr &obstacles_msg);
    ...
    std::shared_ptr<dynamic_reconfigure::Server<VirtualLayer2Config>> _dsrv; // dynamic_reconfigure server for the costmap
